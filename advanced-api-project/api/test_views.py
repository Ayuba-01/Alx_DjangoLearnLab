...
class BookAPITests(APITestCase):
    ...
    def test_detail_book_public_ok(self):
        ...
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(res.data["title"], "The Hobbit")
    ...
    def test_create_book_with_auth_201(self):
        ...
        res = self.client_auth(self.user_token).post(url, payload, format="json")
    ...
    def test_update_book_with_future_year_400(self):
        ...
        res = self.client_auth(self.user_token).patch(url, payload, format="json")
    ...
    def test_delete_book_204(self):
        url = reverse("book-delete", args=[self.b2.pk])
        res = self.client_auth(self.user_token).delete(url)
    ...
    def test_filter_by_author_id(self):
        url = reverse("book-list")
        res = self.client.get(url, {"author": self.a1.id})
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_filter_by_year_range(self):
        url = reverse("book-list")
        res = self.client.get(url, {"publication_year__gte": 1950, "publication_year__lte": 1960})
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_filter_title_contains(self):
        url = reverse("book-list")
        res = self.client.get(url, {"title__icontains": "lord"})
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertTrue(any("Lord" in b["title"] for b in res.data))

    # ---------- Search ----------
    def test_search_by_author_name(self):
        url = reverse("book-list")
        res = self.client.get(url, {"search": "tolkien"})
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertTrue(all("tolkien" in b["author"]["name"].lower() for b in res.data))
